...
사용자는 외부 네트워크 호출(API 키, 과금) 없이, 이미 존재하는 로컬 결과물(`closed-world`, `machine-final`, `inferred-recovery`)만 조합하여 재생 흐름을 확인할 수 있는 하이브리드 미리보기 패키지를 얻고자 한다. 이 결과는 `autonomous-release`, 사람 검증 또는 `final`과 동등하지 않다.
이 문서는 이를 구현하기 위해 기존 파이프라인을 어떻게 수정해야 할지 지시하는 실행 프롬프트이다.

# Goal
전용 `build-offline-hybrid` 명령으로 API 호출 없이 13개 작품의 전체 블록을 커버하는 `source-faithful` 및 `viewer-complete` **미리보기** SRT 패키지를 생성한다. 사람 검수·직접 청취·독립 비평이 없으므로 자동 `final` 승격은 금지한다.
...
# Output
1. API 호출 없는 13개 작품 병합 처리를 실행하는 기존 `build-offline-hybrid` 명령 사용
2. 각 블록의 출처·구조·검증 결과와 `human_final_allowed=false`, `final_promotion_allowed=false`를 기록
3. 실행 후 13개 작품에 대한 `offline-hybrid-preview-v1` 패키지 생성
