...
- `process-title --translation-architecture scene_v2` is an opt-in path with separate semantic reconstruction, dialogue realization, independent source-faithful output, semantic and Korean-dialogue critics, targeted repair, artifact/hash recording, resume, and SRT structure-lock validation. Its fake-provider integration coverage verifies `off`/`metadata` pixel count 0 and `targeted` observation only on the exact semantic trigger. Evidence: `scene_translation.py`, `process_title.py`, `tests/integration/test_scene_v2_process_title.py`, scene contract/evaluation tests. Implemented and synthetic-test verified; `experimental-unbenchmarked`; checked 2026-08-31.

## Not implemented

- No human naturalness benchmark or user-supplied Gemini/external baseline comparison has been run for `scene_v2`. Its current `experimental-unbenchmarked` status is not evidence of superiority. Checked 2026-08-31.
